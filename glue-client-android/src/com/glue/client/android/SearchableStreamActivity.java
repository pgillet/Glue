...
public class SearchableStreamActivity extends ListActivity {
	...
	lv.setOnItemClickListener(new OnItemClickListener() {
		...
		String streamId = ((Event) view.getTag()).getId();
     ...
     * */
    class SearchStreamsTask extends AsyncTask<Void, Void, List<Event>> {
	private final StreamAdapter adapter;
	private final ProgressDialog dialog = new ProgressDialog(
		SearchableStreamActivity.this);
	...
	protected List<Event> doInBackground(Void... params) {
	    ...
	    try {
		// Search for streams
		result = glue.searchEvents(query);
