...
public class JavaTimeResultFormatter implements ResultFormatter {
	...
	/*
	 * loop through the timexes to create two treemaps: - one containing
	 * startingposition=>timex tuples for eradication of overlapping timexes
	 * - one containing endposition=>timex tuples for assembly of the XML
	 * file
	 */
	FSIterator iterTimex = jcas.getAnnotationIndex(Timex3.type).iterator();

	forwardTimexes.clear();
	...
	for (Integer begin : forwardTimexes.navigableKeySet()) {
		...
		if (prevT.getTimexValue().length() > thisT.getTimexValue()
			.length()) {
		    ...
		    removedT = prevT;
		    prevT = thisT; // this iteration's prevT was removed;
		...
		l.log(Level.WARNING,
			"Two overlapping Timexes have been discovered:"
				+ System.getProperty("line.separator")
				+ "Timex A: "
				+ prevT.getCoveredText()
				+ " [\""
				+ prevT.getTimexValue()
				+ "\" / "
				+ prevT.getBegin()
				...
				+ System.getProperty("line.separator")
				+ "Timex B: "
				+ removedT.getCoveredText()
				+ " [\""
				+ removedT.getTimexValue()
				+ "\" / "
				+ removedT.getBegin()
				+ ":"
				+ removedT.getEnd()
	...
	for (Integer docOffset = 0; docOffset <= documentText.length(); docOffset++) {
	    ...
	    /**
	     * grab a new interval/timex if this offset marks the beginning of
	     * one
	     */
	    ...
	    /**
	     * if an interval/timex begin here, append the opening tag. interval
	     * first, timex afterwards
	     */
		...
		if (!interval.getTimexValueEB().equals(""))
		    intervalTag += " earliestBegin=\""
			    + interval.getTimexValueEB() + "\"";
		if (!interval.getTimexValueLB().equals(""))
		    intervalTag += " latestBegin=\""
			    + interval.getTimexValueLB() + "\"";
		if (!interval.getTimexValueEE().equals(""))
		    intervalTag += " earliestEnd=\""
			    + interval.getTimexValueEE() + "\"";
    ...
    public Map<Integer, Timex3> getTimexes() {
	return forwardTimexes;
    }
