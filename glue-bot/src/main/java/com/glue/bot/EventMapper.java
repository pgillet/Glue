...
public class EventMapper implements HtmlMapper<Event> {
	...
	public static final Logger LOG = LoggerFactory.getLogger(EventMapper.class);
	...
	public EventMapper(EventSelectors selectors, Event eventTemplate) throws Exception {
	...
	public Event parse(Element e) throws Exception {
		...
		context.put(SelectorKeys.DESCRIPTION_SELECTOR_KEY, selectors.getDescription());
	...
	private void validate() {
		...
		if (venueSelectors != null) {
			...
			if (venueName == null || venueName.length() == 0) {
				...
				// TODO: Jackson deserializes a VenueSelectors value
				// with empty properties instead of a null object
				selectors.setVenueSelectors(null);
			}
		}
	}
