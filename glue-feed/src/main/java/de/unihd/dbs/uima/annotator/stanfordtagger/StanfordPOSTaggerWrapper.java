...
/**
 * THIS CLASS SHOULD BE DELETED WITH THE NEXT MAJOR HEIDELTIME RELEASE.
 *
 */
public class StanfordPOSTaggerWrapper extends JCasAnnotator_ImplBase {
    private Class<?> component = this.getClass();

    // definitions of what names these parameters have in the wrapper's
    // descriptor file
	...
	annotate_tokens = (Boolean) aContext
		.getConfigParameterValue(PARAM_ANNOTATE_TOKENS);
	annotate_sentences = (Boolean) aContext
		.getConfigParameterValue(PARAM_ANNOTATE_SENTENCES);
	annotate_partofspeech = (Boolean) aContext
		.getConfigParameterValue(PARAM_ANNOTATE_PARTOFSPEECH);
	model_path = (String) aContext
		.getConfigParameterValue(PARAM_MODEL_PATH);
	config_path = (String) aContext
		.getConfigParameterValue(PARAM_CONFIG_PATH);

	// check if the model file exists
	if (model_path == null/* || (new File(model_path)).exists() == false */) { // The
										   // famous
										   // fix
										   // !
	    Logger.printError(component,
		    "The supplied model file for the Stanford Tagger could not be found.");
	    System.exit(-1);
	}
	...
	try {
		...
		mt = new MaxentTagger(model_path, new TaggerConfig("-model",
			model_path), false);
	    }
	} catch (Exception e) {
	    e.printStackTrace();
	    Logger.printError(component,
		    "MaxentTagger could not be instantiated with the supplied model("
			    + model_path + ") and config(" + config_path
			    + ") file.");
	    System.exit(-1);
	}
    }
	...
	Integer offset = 0; // a cursor of sorts to keep up with the position in
			    // the document text
	...
	List<List<HasWord>> tokenArray = MaxentTagger.tokenizeText(
		new StringReader(docText), fac);
	...
	for (List<HasWord> sentenceToken : tokenArray) {
	    ...
	    // create a sentence object. gets added to index or discarded
		...
		// if pos is supposed to be added, iterate through the tagged
		// tokens and set pos
		...
		if (docText.indexOf(thisWord, offset) < 0) {
		    Logger.printDetail(
			    component,
			    "A previously tagged token wasn't found in the document text: \""
				    + thisWord
				    + "\". "
		    ...
		    offset = docText.indexOf(thisWord, offset); // set cursor to
								// the starting
								// position of
								// token in
								// docText
	...
	while (fsi.hasNext()) {
		...
		System.err.println("Sentence: " + s.getBegin() + ":"
			+ s.getEnd() + " = " + s.getCoveredText());
		System.err.println("wrong index in text: "
