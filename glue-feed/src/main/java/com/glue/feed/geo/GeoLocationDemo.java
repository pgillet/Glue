 ...
 * http://JanMatuschek.de/LatitudeLongitudeBoundingCoordinates#Java</a> for the
 * GeoLocation class referenced from this code.
 * </p>
 * 
...
public class GeoLocationDemo {
	...
	java.sql.PreparedStatement statement = connection
		.prepareStatement("SELECT * FROM Places WHERE (Lat >= ? AND Lat <= ?) AND (Lon >= ? "
	...
	java.sql.ResultSet resultSet = findPlacesWithinDistance(earthRadius,
		myLocation, distance, connection);
